...
public class IOSDateSheet extends IBaseSheet implements PickerView.onSelectListener {
    ...
    public IOSDateSheet(Activity activity, int theme, com.seven.lib_common.listener.OnClickListener listener) {
        super(activity, theme, listener);
    }

    public IOSDateSheet(Activity activity, int model, int theme, com.seven.lib_common.listener.OnClickListener listener) {
    ...
    public void setDateSection(String start, String end) {
        ...
        long startTime = TimeUtils.dataToMillisecondMinute(start);
        long endTime =TimeUtils.dataToMillisecondMinute(end);
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.date_sure_rl) //确定选择时间
            listener.onClick(v, selectCalendar.getTimeInMillis());
